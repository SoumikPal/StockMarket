...
export class StockComponent implements OnInit {
  ...
  getbyid(){
    this.http.get(String('http://localhost:8080/stock/'+this.searchid1)).subscribe(response =>this.res=response);
  }
  getbycompanyid(){
    this.http.get(String('http://localhost:8080/stock/stockbycompanyid/'+this.searchid2)).subscribe(response =>this.res2=response);
  }
  getbyexchange(){
    this.http.get(String('http://localhost:8080/stock/stockbyexchangeid/'+this.searchid4)).subscribe(response =>this.res3=response);
  }
  additem(){
    ...
    this.http.post<any>('http://localhost:8080/stock',this.newstock).toPromise().then((d:any)=>{
      console.log(d);
